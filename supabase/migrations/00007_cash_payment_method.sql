-- Migration 00007: Add cash payment support
-- Adds payment_method column to orders and updates create_order RPC
-- to skip wallet debit for cash orders.
...
-- 2. Replace create_order to support p_payment_method param
CREATE OR REPLACE FUNCTION create_order(
    ...
    p_payment_method TEXT DEFAULT 'cash'   -- NEW: 'cash' | 'wallet'
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- Fallback: pick any active pricing rule if no service area match
    ...
    ELSE
        -- Fallback pricing: NGN 500 base + 100/km
    ...
    -- Generate delivery code and expiry
    v_delivery_code := LPAD(FLOOR(RANDOM() * 1000000)::TEXT, 6, '0');
    v_expires_at    := NOW() + INTERVAL '10 minutes';

    -- Wallet payment: debit customer now
    ...
    END IF;
    -- Cash payment: rider collects on delivery — no wallet debit here
